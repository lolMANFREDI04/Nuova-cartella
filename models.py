...
class Client(db.Model):
    ...
    sla = Column(Integer, ForeignKey('sla.id_SLA'), nullable=True, unique=True, default=None)
    ...
    slaRelationship = relationship("SLA",backref="sla",uselist=False, foreign_keys=[sla])
...
class TestTable(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    string_value = db.Column(db.String(255), nullable=False)

class SLA(db.Model):
    id_SLA = db.Column(db.Integer, primary_key=True)
    
    client_id = Column(Integer, ForeignKey('client.client_id'), unique=True)

    client_sla = relationship("Client", backref="client_sla", uselist=False, foreign_keys=[client_id])

    SLA_type_id = db.Column(Integer, ForeignKey('sla_type.id_type'))

    SLA_type = relationship('SLA_type', back_populates='SLA')
...
class SLA_type(db.Model):
    id_type = db.Column(db.Integer, primary_key=True)

    piano_ore = db.Column(db.String(255), nullable=False)

    SLA = relationship('SLA', back_populates='SLA_type')